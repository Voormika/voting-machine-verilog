...
This project implements a digital voting machine in Verilog HDL, designed for simulation on platforms like EDA Playground and targeting hardware like the ZedBoard (Zynq-7000 SoC). 
It demonstrates a mode-based voting system using push buttons and LEDs for input and output. 
The design emphasizes button debouncing, vote logging, and result display.
...
**The testbench**: 
Provides input stimulus for clock, reset, mode, and candidate buttons.Tests both voting and result display functionalities. Monitors LED outputs to verify correct behavior

**Timing Considerations**:
A vote is registered only when a button is pressed continuously long enough (determined by counter reaching value 10). This debouncing mechanism ensures accidental or noisy inputs are ignored
